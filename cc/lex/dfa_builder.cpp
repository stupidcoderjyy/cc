...
void DFABuilder::OutputData(DFASetter& setter) const {
    ...
    // 常数信息
    setter.DFASetCharClassCount(class_count_);
    setter.DFASetStatesCount(static_cast<int>(dfa_.states.size()));
    setter.DFASetStartState(dfa_.start_state);

    // 输出字符到字符类的映射
    for (int ch = 0; ch < kMaxChars; ++ch) {
        if (int class_id = char_to_class_[ch]; class_id > 0) {
            setter.DFASetCharToClass(ch, class_id);
        }
    }
    ...
    for (const auto& state_ptr : dfa_.states) {
        ...
        // 设置状态接受信息
        setter.DFASetStateInfo(state.id, state.is_accepted, state.token);
        ...
        for (int cid = 0; cid < class_count_; ++cid) {
            if (int target = state.class_id_to_next[cid]; target != -1) {
                setter.DFASetGoto(state.id, cid, target);
...
void DFABuilder::PrintCharToClass() const {
    ...
    for (int clazz = 0; clazz < class_count_; ++clazz) {
        ...
        for (const auto& ch : class_to_char[clazz]) {
            std::cout << common::print::DisplayChar(ch) << " ";
        }
        std::cout << '\n';
    }
    std::cout << std::endl;
...
void DFABuilder::PrintState(const DFAState& state) {
    if (state.is_accepted) {
        // 粗体蓝色 "ACCEPTED"
        std::cout << Styled("ACCEPTED", kFgBlack, kBgBlue);
    } else {
        // 粗体黄色 "NON ACCEPTED"
        std::cout << Styled("NON ACCEPTED", kFgBlack, kBgYellow);
    }
    std::cout << " " << state.id;
    if (state.is_accepted) {
...
void DFABuilder::PrintGoto(int input_class, int target, bool target_accept) {
    ...
    // 目标状态显示
    if (target_accept) {
        std::cout << Styled(" " + std::to_string(target) + " ", kFgBlack, kBgWhite);
    } else {
        std::cout << " " << target << " ";
    }
    std::cout << std::endl;
...
void DFABuilder::WarnTokenMatchConflict(DFAState& state, std::vector<std::string>& tokens) {
    Begin(std::cerr, kFgBlack, kBgRed);
