    ...
    for (int lalr_s = 0; lalr_s < num_lalr; ++lalr_s) {
        for (const auto& [sym_id, target] : lr0_states_[lalr_s].transitions) {
            if (const auto& sym = id_to_symbol_[sym_id];
                ...
                if (print_debug_info_) {
                    PrintShift(lalr_s, sym, target);
                }
            } else {
                goto_[lalr_s][sym_id] = target;
                if (print_debug_info_) {
                    PrintGoto(lalr_s, sym, target);
                }
            }
        }
    }
...
void LALRBuilder::EmitActionReduce(int lr_state, const Item& item, const Symbol& forward) {
    ...
    if (item.prod_id == 0) {
        ...
        if (print_debug_info_) PrintAccept(lr_state, forward);
        return;
    }
    const auto& prod = syntax_->productions()[item.prod_id];
    ...
    switch (auto& cell = action_[lr_state][sym_id]; cell.type) {
        case ActionType::kUndefined: {
            ...
            if (print_debug_info_) PrintReduce(lr_state, forward, prod);
            break;
        }
        case ActionType::kShift: {
        ...
        case ActionType::kReduce: {
            ...
            if (existing_prod.priority > new_prod.priority) {
                ...
                throw std::runtime_error("unhandled Reduce-Reduce Conflict");
            }
            cell = {ActionType::kReduce, chosen_id};
            if (print_debug_info_) {
                PrintReduce(lr_state, forward, syntax_->productions()[chosen_id]);
            }
            if (print_conflict_info_) {
...
void LALRBuilder::OutputData(LanguageSetter& setter) const {
    // 产生式
    setter.LALRSetProductions(syntax_->productions());
    setter.LALRSetStatesCount(static_cast<int>(action_.size()));
    // 符号表
    ...
    setter.LALRSetTerminals(terms);
    setter.LALRSetNonTerminals(non_terms);
    ...
    for (int s = 0; s < static_cast<int>(action_.size()); ++s) {
        for (int sym = 0; sym < static_cast<int>(action_[s].size()); ++sym) {
            if (const auto& [type, target] = action_[s][sym]; type != ActionType::kUndefined) {
                setter.LALRSetAction(s, sym, static_cast<int>(type), target);
            }
        }
        for (const auto& [sym, target] : goto_[s]) {
            setter.LALRSetGoto(s, sym - static_cast<int>(terms.size()) - 1, target);
        }
    }
...
void LALRBuilder::PrintGroupInfo(int groupId, const std::set<Item>& items) const {
    ...
    for (const auto& item : items) {
        std::cout << Styled(ItemString(groupId, item), kFgGreen) << '\n';
    }
}

void LALRBuilder::PrintReduce(int group_id, const Symbol& forward, const Production& prod) {
    // 粗体蓝色 "REDUCE"
    std::cout << PadStyled("REDUCE", 10, Align::kLeft, kFgBlack, kBgBlue);
    std::cout << PadStyled("group:", 6, Align::kLeft)
              << PadStyled(group_id, 10, Align::kLeft, kFgGreen);
    std::cout << PadStyled("forward:", 8, Align::kLeft)
              << PadStyled(forward.name, 10, Align::kLeft, kFgGreen);
    std::cout << PadStyled("prod:", 5, Align::kLeft) << Styled(prod.ToString(), kFgGreen);
    std::cout << '\n';
}

void LALRBuilder::PrintGoto(int group_id, const Symbol& input, int nextState) {
    // 粗体白色 " GOTO "
    std::cout << PadStyled("GOTO", 10, Align::kLeft, kFgBlack, kBgWhite);
    std::cout << PadStyled("group:", 6, Align::kLeft)
              << PadStyled(group_id, 10, Align::kLeft, kFgGreen);
    std::cout << PadStyled("input:", 8, Align::kLeft)
              << PadStyled(input.name, 10, Align::kLeft, kFgGreen);
    std::cout << PadStyled("to:", 5, Align::kLeft) << Styled(nextState, kFgGreen);
    std::cout << '\n';
}

void LALRBuilder::PrintShift(int group_id, const Symbol& input, int nextState) {
    // 粗体黄色 "SHIFT "
    std::cout << PadStyled("SHIFT", 10, Align::kLeft, kFgBlack, kBgYellow);
    std::cout << PadStyled("group:", 6, Align::kLeft)
              << PadStyled(group_id, 10, Align::kLeft, kFgGreen);
    std::cout << PadStyled("input:", 8, Align::kLeft)
              << PadStyled(input.name, 10, Align::kLeft, kFgGreen);
...
void LALRBuilder::PrintAccept(int group_id, const Symbol& forward) {
    // 粗体紫色 "ACCEPT"
    std::cout << PadStyled("ACCEPT", 10, Align::kLeft, kFgBlack, kBgPurple);
    std::cout << PadStyled("group:", 6, Align::kLeft)
              << PadStyled(group_id, 10, Align::kLeft, kFgGreen);
    std::cout << PadStyled("forward:", 8, Align::kLeft)
              << PadStyled(forward.name, 10, Align::kLeft, kFgGreen);
...
void LALRBuilder::PrintSymbols() const {
    for (int i = 0; i < id_to_symbol_.size(); ++i) {
        auto& s = id_to_symbol_[i];
        std::cout << '[' << i << ']';
        if (s.type == SymbolType::kNonTerminal) {
            std::cout << Styled(s.name, kFgBlue);
        } else {
            std::cout << Styled(s.name, kFgGreen);
        }
        std::cout << ' ';
    }
    std::cout << std::endl;
...
void LALRBuilder::PrintProductions() const {
    for (int i = 0; i < syntax_->productions().size(); ++i) {
        auto& p = syntax_->productions()[i];
        std::cout << '[' << i << "] " << Styled(p.ToString(), kFgPurple) << '\n';
    }
}
